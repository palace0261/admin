...
$(document).ready(function () {
	...
	$('.js-top-search').on('click', () => {
		$('.top_search').addClass('is-active')
	})

	$('.js_top_search__close').on('click', () => {
...
jQuery.fn.currentCenter = function () {
	const topLeftZeroModals = ['js-inquiry-product','js-order-history']
	const modalLayer = this[0];
	const isOrderLayer = topLeftZeroModals.some(modalName => modalLayer.className.includes(modalName));
	this.css({
		position: 'fixed',
		left: `${isOrderLayer ? 0 : `50%`}`,
		top: `${isOrderLayer ? 0 : `50%`}`,
	})

	this.css({
		'margin-left': `${isOrderLayer ? 0 : `-${-this.outerWidth() / 2}px`}`,
		'margin-top': `${isOrderLayer ? 0 : `-${-this.outerHeight() / 2}px`}`,
	})
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data == 'false') {
				...
				var addHtml = `<div class="gnb_allmenu_top">
					<button type="button" class="btn_all_menu_close" onClick="gd_btn_all_menu_close();">전체메뉴닫기</button>
					</div>`
				addHtml += '<ul class="gnb_allmenu_box">'
				$.each(getData, function (categoryKey, categoryVal) {
					$.each(categoryVal, function (key, val) {
						addHtml += `<li>
                        ${
													val.children
														? `<button class="gd-btn js-gd-btn" type="button">${val.cateNm}</button>`
														: `<a class="gd-btn gd-btn-anchor" href="../goods/goods_list.php?cateCd=${val.cateCd}">${val.cateNm}</a>`
												}`
						if (val.children) {
							addHtml += '<ul hidden class="all_depth1">'
							addHtml += `<li><a class="gd-btn  gd-btn-anchor" href="../goods/goods_list.php?cateCd=${val.cateCd}">전체보기</a>`
							$.each(val.children, function (key1, val1) {
								addHtml += `<li>
                                ${
																	val1.children
																		? `<button class="gd-btn js-gd-btn" type="button">${val1.cateNm}</button>`
																		: `<a class="gd-btn  gd-btn-anchor" href="../goods/goods_list.php?cateCd=${val1.cateCd}">${val1.cateNm}</a>`
																}`
								if (val1.children) {
									addHtml += '<ul hidden class="all_depth2">'
									addHtml += `<li><a class="gd-btn  gd-btn-anchor" href="../goods/goods_list.php?cateCd=${val1.cateCd}">전체보기</a>`
									$.each(val1.children, function (key2, val2) {
										addHtml += `<li>
                                        ${
																					val2.children
																						? `<button class="gd-btn js-gd-btn" type="button">${val2.cateNm}</button>`
																						: `<a class="gd-btn  gd-btn-anchor" href="../goods/goods_list.php?cateCd=${val2.cateCd}">${val2.cateNm}</a>`
																				}
                                        `
										if (val2.children) {
											addHtml += '<ul hidden class="all_depth3">'
											addHtml += `<li><a class="gd-btn  gd-btn-anchor" href="../goods/goods_list.php?cateCd=${val2.cateCd}">전체보기</a>`
											$.each(val2.children, function (key3, val3) {
												addHtml +=
													'<li><a class="gd-btn  gd-btn-anchor" href="../goods/goods_list.php?cateCd=' +
													val3.cateCd +
													'">' +
													val3.cateNm +
													'</a></li>'
				...
				addHtml += '</ul>'
				$('.gnb_allmenu_wrap').html(addHtml).stop(true, true).slideDown('fast')
...
$(document).ready(function () {
	...
	$('.gnb_allmenu_wrap').on('click', '.js-gd-btn', function () {
		const hasHiddenAttr = $(this).siblings()[0].hasAttribute('hidden')
		const $ul = $(this).siblings()

		$ul.attr('hidden', !hasHiddenAttr).parent().toggleClass('is-open')
	})
})
